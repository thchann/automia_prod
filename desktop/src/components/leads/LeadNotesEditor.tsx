  ...
  return (
    <div className="lead-notes-editor flex max-h-[min(75vh,calc(90vh-12rem))] min-h-[min(70vh,28rem)] flex-1 flex-col gap-0 overflow-hidden rounded-xl border border-border/90 bg-[hsl(42_12%_92%)] shadow-inner dark:bg-zinc-950/50">
      <div className="lead-notes-toolbar sticky top-0 z-20 flex shrink-0 flex-col gap-0 border-b border-border/80 bg-[hsl(42_10%_94%)]/95 px-2 py-2 backdrop-blur-md dark:bg-zinc-900/95">
        <Collapsible open={moreFormattingOpen} onOpenChange={setMoreFormattingOpen}>
          <div className="flex flex-wrap items-center gap-2">
            <ToolbarGroup className="flex-1 min-w-0">
              ...
              >
                <SelectTrigger className={cn(compactSelectTrigger, "w-[min(128px,100%)]")} aria-label={tx("Text style", "Estilo de texto")}>
              ...
              >
                <SelectTrigger className={cn(compactSelectTrigger, "w-[min(104px,100%)]")} aria-label={tx("Font", "Fuente")}>
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
                  {FONT_OPTIONS.map((o) => (
            ...
            <CollapsibleTrigger asChild>
              <Button
                ...
                variant="outline"
                size="sm"
                className="h-8 shrink-0 gap-1.5 border-border/90 px-2"
                aria-expanded={moreFormattingOpen}
                aria-label={tx("More formatting and tools", "Mas formato y herramientas")}
              >
                <span className="hidden text-xs font-medium sm:inline">{tx("More", "Mas")}</span>
                <ChevronDown
                  className={cn("h-4 w-4 shrink-0 text-muted-foreground transition-transform duration-200", moreFormattingOpen && "rotate-180")}
                />
              </Button>
            </CollapsibleTrigger>
          </div>
          ...
          <CollapsibleContent className="overflow-hidden data-[state=closed]:animate-collapsible-up data-[state=open]:animate-collapsible-down">
            <div className="mt-2 flex flex-col gap-2 border-t border-border/70 pt-2">
              <div className="flex flex-wrap items-center gap-2">
                ...
                <ToolbarGroup className="sm:ml-0">
                  <Toggle
                    size="sm"
