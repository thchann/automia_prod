...
export interface StatusActivityChartProps {
  ...
  /** Optional explicit groups for status tiles (supports zero-count statuses). */
  statusTileGroups?: StatusActivityTileGroup[];
...
export interface StatusActivityTileGroup {
  key: string;
  color: string;
  name: string;
  count: number;
...
export function StatusActivityChart({
  ...
  const inferredStatusGroups: StatusGroup[] = useMemo(() => {
  ...
  const statusGroups: StatusGroup[] = useMemo(() => {
    if (statusTileGroups && statusTileGroups.length > 0) return statusTileGroups;
    return inferredStatusGroups;
  }, [statusTileGroups, inferredStatusGroups]);
          ...
          {hasPaging && (
            <div className="flex flex-col items-center justify-center gap-1">
              <button
                ...
                className={cn(
                  "inline-flex h-10 w-10 items-center justify-center rounded-full border border-border bg-card text-foreground transition-colors",
